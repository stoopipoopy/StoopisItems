        ...
        if (itemMeta.hasLore()) {
            ...
            if ((e.getAction() == Action.RIGHT_CLICK_BLOCK || e.getAction() == Action.RIGHT_CLICK_AIR) && IDLORE.equals(" Hellfire.")) {
                if (this.magmaCooldown.containsKey(p.getUniqueId())) {
                    ...
                    final String command = "particle flame " + center.getLocation().getBlockX() + " " + center.getLocation().getBlockY() + " " + center.getLocation().getBlockZ() + " 2 80 2 0 3 force";
                    this.magmaPillarCooldown.put(p.getUniqueId(), System.currentTimeMillis());

                    new magmaPillarManager(world, center, attunements, magmaPillarCooldown, p, e, magmaPillarCooldownTime, console,command,this).runTaskAsynchronously(this);
